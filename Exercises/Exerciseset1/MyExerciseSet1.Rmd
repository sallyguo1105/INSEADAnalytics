...
**Your Answers here:**
<br>AAPL ticket was pulled by the file named dataSet1.R. 
It was pulled by the array function c("SPY", "AAPL")
mytickers = c("SPY", "AAPL")
mean(AAPL) [1] 0.001478376
sum(AAPL) [1] 5.954898
sd(AAPL,na.rm=FALSE) [1] 0.02390385
To get Yahoo's Data, we have to modify the array in dataSet1.R to include Yahoo's ticker. The array becomes:
...
**Your Answers here:**
<br>To get number of rows, we use the command:
nrow(transposedData)
...
**Your Answers here:**
<br>The program does not include the first data point. To include 2001-01-01, we must change the start date variable to the day before in file dataSet1.R
startDate = "2000-12-31"

We can get returns of other companies simply by changing the array code. For example, the command below will get you ticker data from HSBC
mytickers = c("HSBC") get 
